...
$checkEmail = mysqli_query($con, "SELECT id FROM admin2 WHERE email = '$email' AND id != '$id'");
if (mysqli_num_rows($checkEmail) > 0) {
    echo json_encode(['success' => false, 'message' => 'Email already exists']);
    exit;
}
// Fetch current hashed password
$PasswordResult = mysqli_query($con, "SELECT password FROM admin2 WHERE id = '$id'");
...
if (!empty($newPassword)) {
    ...
    $checkPassword = mysqli_query($con, "SELECT password FROM admin2");
